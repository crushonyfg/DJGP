...
def predict_vi(regions,
    ...
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    X_test = hyperparams['X_test'].to(device)     # [T, D]
    T = X_test.shape[0]
    Z      = hyperparams['Z'].to(device)
    mu_V   = V_params['mu_V'].to(device)          # [m2, Q, D]
    sigma_V= V_params['sigma_V'].to(device)
    ...
    mu_W, _ = qW_from_qV(
        X_test, Z,
        mu_V, sigma_V,
        hyperparams['lengthscales'].to(device),
        hyperparams['var_w'].to(device)
    )  # mu_W: [T, Q, D]
    ...
    # 3) 对每个 region 直接用 Wj = mu_W[j] 来预测
    for j, r in enumerate(regions):
        Xj = r['X'].to(device)          # [n_j, D]
        yj = r['y'].to(device).view(-1,1)  # [n_j, 1]

        # 模式解：直接取均值
        Wj = mu_W[j]                    # [Q, D]
        # （可选）归一化每一行
        # Wj = F.normalize(Wj, p=2, dim=1)  # [Q, D]
...
if __name__=="__main__":
    ...
    # mu_p, var_p = predict_vi(regions, V_params, u_params, hyperparams)
    print("mu_pred=", mu_p)
